...
export default function MenuDropdown({ postId }: { postId: string }) {
      ...
      >
        <Menu.Items className="absolute p-2 mt-1 space-y-2 bg-black rounded-md -right-1 w-36 shadow-app-shadow">
          ...
          <Menu.Item>
            {({ active }) => (
              <button
                className={`flex w-full gap-2 items-center duration-200 text-red-500 px-2 py-2 rounded-md text-sm ${
                  active && 'bg-gray-900'
                }`}
              >
                <svg
